...
const editClickHandle = async(row:any) => {
  ...
  const result = await findActiveById('/findActiveById',editUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteActive('/deleteActive',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addActive('/addActive',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateActive('/updateActive',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getActivesRequest("/findActives", {
