...
// 导入网络方法
import { getArticlesRequest,deleteArticle,addArticle,updateArticle,findArticleById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑文章");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findArticleById('/findArticleById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findArticleById('/findArticleById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteArticle('/deleteArticle',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addArticle('/addArticle',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateArticle('/updateArticle',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getArticlesRequest("/findArticles", {
...
const getPermission = () => {
  ...
  const articleData = menuList.find(item => item.name === 'article')
  if(articleData){
