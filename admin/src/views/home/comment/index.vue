<template>
  <div class="admin">
    ...
    <page-table 
      ...
      <!-- 插槽部分处理 -->
      <!-- 评论类型列 -->
      <template #commentType="scope">
        <el-tag v-if="scope.row.commentType === 0">
          文章
        </el-tag>
        <el-tag v-else-if="scope.row.commentType === 1">
          视频
        </el-tag>
        <el-tag v-else-if="scope.row.commentType === 2">
          活动
        </el-tag>
        <el-tag v-else>
          其他
      ...
      <!-- 评审状态列 -->
      <!-- 收藏数量列 -->
      <template #supportUser="scope">
        <el-tag>
          {{ scope.row.supportUser.length }}
        </el-tag>
      </template>
      <!-- 收藏数量列 -->
      <!-- 时间列 -->
      ...
      <template #handle="scope">
          ...
          <el-button
            v-if="permission.audit && scope.row.status === 0"
            type="primary" 
            size="small"
            :icon="Setting" 
            @click="auditClickHandle(scope.row)">
              审核通过
          </el-button>
          <el-button 
...
// 导入网络方法
import { getCommentsRequest,deleteComment,addComment,updateComment,findCommentById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑评论");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findCommentById('/findCommentById',editUserId.value)
  if(result.code === 200){
...
// 审核按钮点击触发
const auditClickHandle = async(row:any) => {
  const reuslt = await updateComment('/updateComment',{
    status: 1
  },row.id)
  if (reuslt.code === 200) {
    // 弹窗提示
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findCommentById('/findCommentById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteComment('/deleteComment',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addComment('/addComment',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateComment('/updateComment',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getCommentsRequest("/findComments", {
...
const getPermission = () => {
  ...
  const articleData = menuList.find(item => item.name === 'comment')
  if(articleData){
