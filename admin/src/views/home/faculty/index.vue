...
// 导入网络方法
import { getFacultiesRequest,deleteFaculty,addFaculty,updateFaculty,findFacultyById } from "@/network/home";
// 导入pinia
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findFacultyById('/findFacultyById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findFacultyById('/findFacultyById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteFaculty('/deleteFaculty',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addFaculty('/addFaculty',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateFaculty('/updateFaculty',{
