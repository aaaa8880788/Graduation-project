...
// 导入网络方法
import { getGiftsRequest,deleteGift,addGift,updateGift,findGiftById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑礼品");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findGiftById('/findGiftById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findGiftById('/findGiftById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteGift('/deleteGift',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateGift('/updateGift',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getGiftsRequest("/findGifts", {
...
const getPermission = () => {
  ...
  const giftData = menuList.find(item => item.name === 'gift')
  if(giftData){
