  ...
  <div class="admin">
    ...
    <page-table 
      ...
      <template #image="scope">
        <imagePreview 
          v-if="scope.row.giftData.image"
          :imageUrl="scope.row.giftData.image" />
        <img
          v-else 
          style="width: auto; height: 30px"
          src="@/assets/image/not_image.png" />
      </template>
      <!-- 订单图片列 -->
      <!-- 发货状态栏 -->
...
// 导入网络方法
import { getOrdersRequest,deleteOrder,addOrder,updateOrder,findOrderById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑订单");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findOrderById('/findOrderById',editUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteOrder('/deleteOrder',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addOrder('/addOrder',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateOrder('/updateOrder',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getOrdersRequest("/findOrders", {
...
const getPermission = () => {
  ...
  const giftData = menuList.find(item => item.name === 'order')
  if(giftData){
    const obj:Record<string,boolean> = {}
    const permission:string[] = giftData.permission
