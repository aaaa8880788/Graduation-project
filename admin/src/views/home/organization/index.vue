...
// 导入网络方法
import { getOrganizationsRequest,deleteOrganization,addOrganization,updateOrganization,findOrganizationById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑学院");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findOrganizationById('/findOrganizationById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findOrganizationById('/findOrganizationById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteOrganization('/deleteOrganization',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addOrganization('/addOrganization',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateOrganization('/updateOrganization',{
...
const getPermission = () => {
  ...
  const organizationData = menuList.find(item => item.name === 'organization')
  if(organizationData){
    const obj:Record<string,boolean> = {}
    const permission:string[] = organizationData.permission
