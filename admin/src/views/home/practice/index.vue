...
// 导入网络方法
import { getPracticesRequest,deletePractice,addPractice,updatePractice,findPracticeById } from "@/network/home";
// 导入pinia
...
const newAddDialogConfig = computed(() => {
  ...
  if(!addDialogFormData.value){
    return config
  }
  if(addDialogFormData.value.options.length){
    options = addDialogFormData.value.options.filter((item: string) => item !== '').map((item:string,index:number)=>{
      return {
        label:item,
        value:item,
        key:index,
  ...
  const formItem = config.formItems.find(item => item.field === 'answer')
  if(formItem){
...
const newEditDialogConfig = computed(() => {
  ...
  if(!editDialogFormData.value){
    return config
  }
  if(editDialogFormData.value.options.length){
    options = editDialogFormData.value.options.filter((item: string) => item !== '').map((item:string,index:number)=>{
      return {
        label:item,
        value:item,
        key:index,
...
const newDetailDialogConfig = computed(() => {
  ...
  if(!detailDialogFormData.value){
    return config
  }
  if(detailDialogFormData.value.options.length){
    options = detailDialogFormData.value.options.filter((item: string) => item !== '').map((item:string,index:number)=>{
      return {
        label:item,
        value:item,
        key:index,
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findPracticeById('/findPracticeById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findPracticeById('/findPracticeById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deletePractice('/deletePractice',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addPractice('/addPractice',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updatePractice('/updatePractice',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getPracticesRequest("/findPractices", {
...
const getPermission = () => {
  ...
  const practiceData = menuList.find(item => item.name === 'practice')
  if(practiceData){
    const obj:Record<string,boolean> = {}
    const permission:string[] = practiceData.permission
