  ...
  <div class="admin">
    ...
    <page-table 
      ...
      <template #request="scope">
        <div v-html="scope.row.request"></div>
      </template>
      <!-- 申请要求列 -->
      <!-- 时间列 -->
...
// 导入网络方法
import { getRolesRequest,deleteRole,addRole,updateRole,findRoleById } from "@/network/home";
// 导入pinia
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findRoleById('/findRoleById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findRoleById('/findRoleById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteRole('/deleteRole',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateRole('/updateRole',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getRolesRequest("/findRoles", {
...
const getPermission = () => {
  ...
  const practiceData = menuList.find(item => item.name === 'role')
  if(practiceData){
