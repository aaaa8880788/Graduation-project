...
// 导入网络方法
import { getSchoolsRequest,deleteSchool,addSchool,updateSchool,findSchoolById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑学校");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findSchoolById('/findSchoolById',editUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteSchool('/deleteSchool',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addSchool('/addSchool',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateSchool('/updateSchool',{
...
const getPermission = () => {
  ...
  const organizationData = menuList.find(item => item.name === 'school')
  if(organizationData){
