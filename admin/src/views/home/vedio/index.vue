...
// 导入网络方法
import { getVedioesRequest,deleteVedio,addVedio,updateVedio,findVedioById } from "@/network/home";
// 导入pinia
...
// 对话框标题
const editDialogTitle = ref("编辑视频");
// 编辑对话框组件ref对象
...
const editClickHandle = async(row:any) => {
  ...
  const result = await findVedioById('/findVedioById',editUserId.value)
  if(result.code === 200){
...
const detailClickHandle = async(row:any) => {
  ...
  const result = await findVedioById('/findVedioById',detailUserId.value)
  if(result.code === 200){
...
const deleteClickHandle = (row: any) => {
  ...
  .then(async () => {
      const reuslt = await deleteVedio('/deleteVedio',{
        id:row.id,
        type:row.type
      })
      if (reuslt.code === 200) {
...
const addDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await addVedio('/addVedio',{
...
const editDialogConfirmHandle = async() => {
  ...
  if (!valid) return;
  let res = await updateVedio('/updateVedio',{
...
const getPageData = async (queryInfo?: object) => {
  ...
  const result = await getVedioesRequest("/findVedioes", {
...
const getPermission = () => {
  ...
  const articleData = menuList.find(item => item.name === 'vedio')
  if(articleData){
